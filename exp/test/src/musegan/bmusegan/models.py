...
class Model(object):
	...
	def __init__(self, sess, config, model_path, resolution=1, name='model'):
		self.sess = sess
		self.name = name
		self.config = config
...
class GAN(Model):
	...
	def __init__(self, sess, config, resolution, model_path, name='GAN', reuse=None):
		super().__init__(sess, config, resolution, model_path, name)
	...
	def build(self):
		...
		data_shape = [(32, 4, 1, 1, 8), (32, 4, 3, 1, 8), (32, 4, 6, 1, 8), (32, 4, 12, 1, 8), 
			(32, 4, 24, 1, 8), (32, 4, 48, 1, 8), (32, 4, 96, 1, 8), (32, 4, 96, 2, 8), (32, 4, 96, 4, 8), 
			(32, 4, 96, 12, 8), (32, 4, 96, 84, 8)]

		# Create placeholders
		self.z = tf.placeholder(tf.float32, (self.config['batch_size'], self.config['net_g']['z_dim']), 'z')
		...
		self.test_bernoulli = self.G.tensor_out > tf.random_uniform(data_shape[self.resolution])
	...
	def zoom_size(resolution):
		if resolution == 1:
			zoom_ = [1, 1, 1/96, 1/84, 1]
		elif resolution == 2:
			zoom_ = [1, 1, 1/32, 1/84, 1]
		elif resolution == 3:
			zoom_ = [1, 1, 1/16, 1/84, 1]
		elif resolution == 4:
			zoom_ = [1, 1, 1/8, 1/84, 1]
		elif resolution == 5:
			zoom_ = [1, 1, 1/4, 1/84, 1]
		elif resolution == 6:
			zoom_ = [1, 1, 1/2, 1/84, 1]
		elif resolution == 7:
			zoom_ = [1, 1, 1, 1/84, 1]
		elif resolution == 8:
			zoom_ = [1, 1, 1, 1/42, 1]
	...
	def train(self, x_train, train_config):
		...
		for epoch in range(train_config['num_epoch']):
			...
			for batch in range(num_batch):
				...
				zoom_ = zoom_size(resolution)

				x_resized = x_train[x_random_batch[batch]]
				x_resized  = zoom(t, zoom=zoom_, order=5, mode='nearest')

				feed_dict_batch = {self.x: x_resized, self.z: z_random_batch[batch]}
...
class RefineGAN(Model):
	...
	def __init__(self, sess, config, resolution, name='RefineGAN', reuse=None):
	...
	def build(self):
		...
		self.R = Refiner(self.pretrained.G.tensor_out, self.config, slope_tensor=self.slope_tensor, name='R')
