...
class NeuralNet(object):
	...
	def __init__(self, tensor_in, resolution, block, architecture=None, condition=None,
		...
		if architecture is not None:
			with tf.variable_scope(name, reuse=reuse) as scope:
				self.scope = scope
				self.layers = self.build(architecture, self.resolution, self.block, self.name)
	...
	def build(self, architecture, resolution, block, name):
		...
		for idx, structure in enumerate(architecture):
			if idx <= resolution and block == 'shared':
				if idx > 0:
				...
				layer = Layer(connected, structure, slope_tensor=self.slope_tensor, name='Layer_{}'.format(idx))
				# if resolution == 0 and name[-1] == '0':
				# 	print(layer)
				# elif resolution == 1:
				print(layer)
				...
				if skip_connection is None:
					...
					connected = tf.concat([prev_layer, layers[structure[4][1]].tensor_out], -1)
				...
				if idx >= 11 - resolution:
					resolution -= 1
					if flag == False:
						prev_layer = self.tensor_in
						flag = True
					else:
						prev_layer = layers[counter-1].tensor_out
					...
					skip_connection = None
					connected = prev_layer		
					layer = Layer(connected, structure, slope_tensor=self.slope_tensor, name='Layer_{}'.format(counter), reuse=False)
					counter += 1
...
for resolution in range(1, 12):
	print("resolution" + str(resolution))
	print("|===============================================Generator===============================================|")
	architecture = MODEL_CONFIG['net_g']['shared']
	tensor_in = tf.placeholder(tf.float32, (32, 128), 'z')
	nets = OrderedDict()
	block = 'shared'
	nets['shared'] = NeuralNet(tensor_in, resolution, block, architecture, name='shared')
	print("|=======================================================================================================|")

	print("|================================================Refiner================================================|")
	architecture = MODEL_CONFIG['net_g']['refiner']
	num_track = 8
	slope_tensor = tf.Variable(1.0)
	tensor_in = nets['shared'].tensor_out
	block = 'refiner'
	nets['refiner'] = [NeuralNet(tf.expand_dims(tensor_in[..., idx], -1), resolution, block, architecture, 
		slope_tensor=slope_tensor, name='refiner'+str(idx)) for idx in range(num_track)]
	tf.concat([nn.layers[-1].preactivated for nn in nets['refiner']], -1)
	a = tf.concat([nn.tensor_out for nn in nets['refiner']], -1)
	print("|=======================================================================================================|")

	print("|=============================================Discriminator=============================================|")
	architecture = MODEL_CONFIG['net_d']['shared']
	tensor_in = a #nets['refiner'].tensor_out
	slope_tensor = None
	nets_d = OrderedDict()
	block = 'd_shared'
	nets_d['shared'] = NeuralNet(tensor_in, resolution, block, architecture, name='shared')
	print("|=======================================================================================================|")
